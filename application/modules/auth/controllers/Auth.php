...
class Auth extends MX_Controller {
	...
	function index() {
		...
		} elseif ( $this->ion_auth->is_companyadmin() ) // remove this elseif if you want to enable this for non-admins
		{
	...
	function _render_page( $view, $data = null, $returnhtml = false )//I think this makes more sense
	{
		...
		if ( $listtitle != 'Login' ) {
			$this->load->view( 'include/header', $this->ion_auth->user()->row() );
			$this->load->view( 'include/sub_header' );
...
// list users
	...
	public function list_user() {
		...
		$pagingConfig        = $this->paginationlib->initPagination( "/auth/list_user/pages/", $this->mymodel->getCount( 'id', 'users', 'id != ' . $this->ion_auth->user()->row()->id ) );
		...
		$this->data['listUsers'] = $this->mymodel->get( 'users', '*', 'id != ' . $this->ion_auth->user()->row()->id, $pagingConfig['per_page'], $pages );
	...
	public function profile( $id ) {
		if ( ( ! $this->ion_auth->logged_in() || ! $this->ion_auth->is_admin() ) && $this->ion_auth->get_user_id() != $id ) {
			// redirect them to the home page because they must be logged in to view this
